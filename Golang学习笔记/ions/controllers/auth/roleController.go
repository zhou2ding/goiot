...
func (r *RoleController) List()  {
	...
	qs.Filter("is_del",0).All(&roles)

	count,_ := qs.Filter("is_del",0).Count()
	qs.Filter("is_del",0).Limit(pagePerNum).Offset(offsetNum).All(&roles)
...
func (r *RoleController) ToRoleUser()  {
	...
	users := []auth.User{}
	if len(role.User) > 0 {
		o.QueryTable("sys_user").Filter("is_del",0).Filter("is_active",1).Exclude("id__in",role.User).All(&users)

	}else {   // 没有绑定的数据
		o.QueryTable("sys_user").Filter("is_del",0).Filter("is_active",1).All(&users)
...
func (r *RoleController) DoRoleAuth()  {
	...
	for _,auth_id := range id_arr{
		auth_id_int,_ := strconv.Atoi(auth_id)
		if auth_id_int !=0 {
