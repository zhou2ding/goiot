...
func init() {
	...
	// "/param/?:id:int"可以限制类型，限制类型是string的话等于没限制；用正则的方式限制类型,int是--id:([\d]+)或id:([0-9]+)，string是id:([\w]+)
	...
	beego.Router("/po2m", &testorm.O2MController{})                  // 一对多实战
	beego.Router("/po2m-add", &testorm.ArticleAddController{})       // 一对多实战--新增
	beego.Router("/po2m-update", &testorm.ArticleUpdateController{}) // 一对一实战--更新

	beego.Router("/pm2m", &testorm.PostListController{})          // 多对多实战
	beego.Router("/tag-list", &testorm.TagListController{})       // 多对多实战
	beego.Router("/tag-add", &testorm.TagAddController{})         // 多对多实战
	beego.Router("/post-add", &testorm.PostAddController{})       // 多对多实战
	beego.Router("/post-update", &testorm.PostUpdateController{}) // 多对多实战
	beego.Router("/post-delete", &testorm.PostDelController{})    // 多对多实战

	beego.Router("/builder", &testorm.TestBuilderController{}) //构造查询
