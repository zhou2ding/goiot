...
func (t *TestOrm4Controller) Get() {
	...
	qs.Filter("title_contains", "西").Filter("author__exact", "意大利").One(&art) //多个筛选条件，相当于mysql中的AND。所有的接口都能这样用

	//All
	arts := []models.Article{}
	qs.Filter("title_contains", "西").All(&arts)                     //查询的满足条件的多个结果扔进arts切片中
	...
	qs.Distinct().All(&arts, "author")                              //因为distinct本来就是只能查需要去重的字段，所以可以这么用
	...
	//先把查询结果扔进arts切片，然后遍历切片，把数据循环插入数据库，然后关闭
