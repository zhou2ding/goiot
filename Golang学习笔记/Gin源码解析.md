...
![img](Gin源码解析.assets/radix_tree.png)
...
![addroute](Gin源码解析.assets/addroute.gif)
...
![gin_middleware1](Gin源码解析.assets/gin_middleware1.png)

我们可以在中间件函数中通过再次调用`c.Next()`实现嵌套调用（func1中调用func2；func2中调用func3），

![gin_middleware2](Gin源码解析.assets/gin_middleware2.png)
...
`c.Set()`和`c.Get()`这两个方法多用于在多个函数之间通过`c`传递数据的，比如我们可以在认证中间件中获取当前请求的相关信息（userID等）通过`c.Set()`存入`c`，然后在后续处理业务逻辑的函数中通过`c.Get()`来获取当前请求的用户。`c`就像是一根绳子，将该次请求相关的所有的函数都串起来了。![gin_middleware3](Gin源码解析.assets/gin_middleware3.png)
...
1. 在for循环中，把len复制给一个变量，这样就只用计算一遍len而不用每次循环都计算

2. for循环中把完全不可能的条件先return或continue掉

3. 当结构体需要实现某个接口，在定义完结构体，就马上用匿名变量定义一个类型是接口、值是结构体指针的变量

4. 初始化时一次性把容量申请到位

5. 拼接两个切片：先计算出最终切片的长度，然后把第一个切片copy进最终切片，再把第二个切片copy进最终切片（拷贝的目的地是从第一个切片已经占了的位置往后开始）

   ![image-20210607165726470](Gin源码解析.assets/image-20210607165726470.png)

# 实战项目

## 项目搭建

![image-20210607215851160](Gin源码解析.assets/image-20210607215851160.png)
