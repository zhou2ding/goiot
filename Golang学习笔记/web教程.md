...
### 超链接标签
  ...
  - 语法规范：\<a>\</a>（anker的缩写）
    ...
    | 属性   | 说明                                                         |
    | ------ | ------------------------------------------------------------ |
    | href   | 用于指定链接目标的url地址，必须属性。`<a href='javascript:;'>`时，点击链接不会发生跳转 |
...
### 表格标签
  ...
  - 合并单元格
    ...
    - 目标单元格
      - 跨行：最上侧单元格为目标单元格，写合并代码rowspan，删除多余的单元格
      - 跨列：最左侧单元格为目标单元格，写合并代码colspan，删除多余的单元格
...
- \<input>标签：是单标签
  ...
  - 必须有type属性，除了下面列的，还是email、tel、url、search、date、time、number、color、range、datetime-local、month、week等等
    ...
    | hidden   | 定义隐藏的输入字段                                           |
    | image    | 定义图像形式的提交按钮，可以有image标签的那些属性            |
  ...
  - 其他属性
    ...
    | placeholder | 用户自定义 | 用于在输入框中显示提示文本                                   |
...
- CSS可以放在style属性`<p style="color: red; font-size: 20px;">demo</p>`、\<style>\</style>标签、css文件` <link rel="stylesheet" href="demo.css">`中，分别称为内联式、内部样式表、外部样式表，CSS规则由两个主要部分构成
  - 选择器：修改谁的样式，用来选择标签的
...
- 一个标签可以有多个类名，之间用空格隔开，这个标签就分别具有这些类名的样式
- 通用的样式可以作为一个类，方便统一修改
...
- 会把和子元素是同种标签（或相同类名）的所有后代都选中（继承性）；如果要选更深层级的子元素，则把后代的子标签挨个跟在后面直到目标层级（虽然不写完也能选中目标元素，但一般写全层级，不容易出问题）
...
- 结构伪类
  ...
  - `:first-child`，选择父元素的第一个子元素（谁在冒号左边，谁就是父元素）
  - `:last-child`，选择父元素的最后一个子元素
  - `:nth-child(n)`，选择父元素的第n个子元素，括号中的n是实际的数字

- 表单伪类

  - `:checked`，选择被选中的`<input>`元素
  - `:disabled`，选择被禁用的`<input>`元素
  - `:enabled`，选择被启用的`<input>`元素

- 其他伪类

  - `:empty`，选择没有子元素的元素
  - `:before`和`:after`，用于在元素前后插入内容
...
- 复合属性：文字属性合并到一起简写
  ...
  - 严格按照以下顺序，中间用空格隔开，line-height可以省略（`/`是写的时候用的符号，不是或的意思），line-height的值是字体的倍数或`normal(默认行高，具体由浏览器决定)`
...
- text-indent：给文本的第一行缩进，值可以是
  ...
  - 像素值px（可以是负值，文本会超出界面）
  - 相对单位em（当前元素一个文字的距离），在大多数情况下，`em` 的值是相对于父元素的字体大小来计算的。例如，如果父元素的字体大小是16px，那么1em等于16px。
...
- 给父元素设置text-align: center，则子元素也都会水平居中
...
- 设置指定角的圆角
  ...
  - 分开设置，使用border-top-left-radius，其他三个角以此类推（不常用）
...
- 字体图标：iconfont，本质上是字体，主要用于显示网页中通用的小图标，解决了精灵图放大缩小会失真、更换复杂的缺点
  ...
  - 使用
    ...
    - 在下载包中的demo.html找到需要的图标，复制它的小方框：或它的编号（注意e要进行转义，即\e），放到盒子中，然后把盒子的font-family设成icomoon

    - 追加字体图标：把已下载包里的selection.json上传到icomoon网站，选择需要的新图标再重新下载

- 三角：网页中一些三角形直接使用css就能画出来，不用做成图片或字体图标

  - 盒子的高宽均是0，设置其中一个方向边框的粗细、颜色、线型属性，其他边框粗细一致，但透明（三角指向哪个方向，就把该方向的反方向设置属性）
...
- 去掉盒子中的图片底部的空白缝隙
  ...
  - 给图片添加vertical-align属性，middle、top、bottom都行
...
- 溢出内容省略号
  ...
  - 单行文本溢出显示省略号
    1. 强制文本一行内显示：`white-space: nowrap;`，默认为normal，自动换行
...
- 常见布局技巧
  ...
  - 多个盒子紧挨，都有细边框，鼠标经过时边框变色：
    - 巧妙运用margin负值：一行内多个浮动的盒子，用负值（绝对值是边框的像素值），可以实现两个相邻边框紧挨着，且贴合出是细边框。
    - 鼠标经过盒子时，改变边框的颜色，因为前面设置了负值，所以盒子的右边框不会变色，解决方法：鼠标经过时，提高盒子的层级，也就是把定位设置成relative（当盒子没有定位时），或鼠标经过时，设置z-index为正值（当盒子有定位时）
  ...
  - 梯形
    - 矩形+直角三角形即可：巧妙运用css三角，把其中三个边设为透明，上边框宽度较大，则右条边框生成的三角形就是一个直角三角形。其他方向的以此类推。
...
> 以前只有div，没有语义，对搜索引擎不友好；在IE9中，需要把这些标签转换为块级元素
>
> ![image-20230706121006815](/Users/zhoudongbin/Library/Application Support/typora-user-images/image-20230706121006815.png)

- header标签：头部标签
- nav标签：导航栏标签
- article标签：内容标签
- section标签：某个区域（大号div）
- aside标签：侧边栏标签
- footer：尾部标签

### 多媒体标签

- audio标签：支持三种格式，mp3、wav、ogg。常用属性如下

  | 属性     | 值       | 描述                         |
  | -------- | -------- | ---------------------------- |
  | autoplay | autoplay | 音频就绪后自动播放，用js解决 |
  | controls | controls | 播放控件                     |
  | loop     | loop     | 是否循环播放                 |
  | src      | url      | 要播放的音频的url            |
  | muted    | muted    | 是否静音播放                 |

- video标签：支持三种视频格式，mp4、webm、ogg，尽量用mp4。常用属性如下（下面属性可以只写属性名字，也可以写属性名字=属性值）

  | 属性     | 值             | 描述                                                         |
  | -------- | -------------- | ------------------------------------------------------------ |
  | autoplay | autoplay       | 视频就绪自动播放，chrome需要添加muted或muted="muted"属性才能自动播放 |
  | controls | controls       | 播放控件，静音、全屏等（正式开发中不用，而是用js）           |
  | width    | 像素值或百分比 | 播放器的宽度                                                 |
  | height   | 像素值或百分比 | 播放器的高度                                                 |
  | loop     | loop           | 是否循环播放                                                 |
  | preload  | auto<br>none   | 是否预先加载属性（如果设置了autoplay，则忽略该属性）         |
  | src      | url            | 视频的地址                                                   |
  | poster   | imgurl         | 加载视频时等待的画面图片                                     |
  | muted    | muted          | 是否静音播放                                                 |
...
| 属性值        | 说明                                                  |
| ------------- | ----------------------------------------------------- |
| type="email"  | 限制用户输入必须为email类型，否则提交表单时会报错提示 |
| type="url"    | url类型                                               |
| type="date"   | 日期类型                                              |
| type="time"   | 时间类型                                              |
| type="month"  | 月类型                                                |
...
| 属性         | 值        | 说明                                                         |
| ------------ | --------- | ------------------------------------------------------------ |
| required     | required  | 表单内容不能为空，否则                                       |
| placeholder  | 提示文本  | 表单的提示信息，存在默认值将不显示                           |
| autofocus    | autofocus | 自动获得焦点                                                 |
| autocomplete | off/on    | 记录之前提交过的表单值，点击输入框时，有下拉框提示输入过的内容。默认on，一般关掉，提高安全性。需要放在表单内同时加上name属性。 |
| multiple     | multiple  | type="file"时，可以多选文件                                  |

设置placeholder提示文本的颜色：
...
可以根据元素特定的属性来选择元素，E是标签名，val也可以不带引号。[]才是属性选择器，E[]则是标签选择器和属性选择器的结合，权重11
...
根据文档结构来选择元素，常用于根据父级选择器里面的子元素，E一般把父子都写上

|      选择符      |                             简介                             |
| :--------------: | :----------------------------------------------------------: |
|  E:first-child   |                 匹配父元素中的第一个字元素E                  |
|   E:last-child   |                匹配父元素中的最后一个字元素E                 |
|  E:nth-child(n)  | 匹配父元素中的一个或多个特定的子元素E，n可以是数字、关键字和公式<br>是数字时，从1开始<br>是关键字时，even偶数，odd奇数<br>是公式时，字母必须是n，如n、n+5、2n、-n+3等，n 从0开始每次加1，第0个或超出元素个数的或负数会被忽略<br>会把所有的子元素都排列序号，匹配时，先看序号，然后看序号左边的标签名，看能不能匹配上 |
| E:first-of-type  |                      指定元素E的第一个                       |
|  E:last-of-type  |                     指定元素E的最后一个                      |
| E:nth-of-type(n) | 指定元素E的第n个<br>会把指定元素的子元素排列序号，匹配时，先看序号左边的标签名，然后看序号，看能不能匹配上<br>n的用法和child一样 |

### 伪元素选择器

可以利用css创建标签元素，而不需要html标签，简化html结构，可以和伪类选择器结合使用，div:hover::before；伪元素还可以用来清除浮动
...
通过box-sizing指定盒模型，值可以为content-box、border-box

- box-sizing: content-box的盒子大小为width+padding+border
- box-sizing: border-box的盒子大小为width
...
将模糊或颜色偏移等图形效果应用于元素，语法格式

`filter: 函数名()`，例如：`filter: blur(5px);`模糊处理，数值越大越模糊

### calc

在声明css属性值时进行一些计算，例如：`width: calc(100%-80px)`，子盒子的宽度永远比父盒子少20px

### 过渡效果

trainsition属性，经常和:hover搭配使用，语法：`transition: 要过渡的属性 花费时间 运动曲线 何时开始`

- 属性：要变化的css属性，如果要所有属性都变化过渡，写all
- 花费时间：单位是秒，比如0.5s
- 运动曲线：默认ease（逐渐慢下来），还有linear（匀速），ease-in（加速），easu-out（减速），ease-in-out（先加速后减速），可省略
- 何时开始：单位是秒，可以设置延迟触发，默认是0s，可省略

使用方法：

- 谁做过渡给谁加
- 多个属性时，用逗号分割，例如：`transition: width .5s, height .5s`
...
- logo里面放一个h1标签，目的是提权，告诉搜索引擎这个地方很重要
- h1里面放一个链接，可以返回首页，把logo的背景图片给链接即可
- 为了被搜索引擎收录，链接里面放文字（网站名称），文字不显示
  - 方法1：text-indent移到盒子外面（text-indent:-9999px），然后overflow:hidden
  - 方法2：直接给font-size: 0

### 其他技巧

- padding一般给a，这样用户点起来比较方便
- 如果一个盒子既有left也有right属性，则默认会执行left，top、bottom同理
