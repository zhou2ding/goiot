...
> Nginx是一个高性能的web服务器和web方向代理服务器，也可以用来做邮件代理服务器，用C语言开发的
>
...
4. 进入安装的目录，有四个文件夹
   ...
   - 关闭
     ...
     - `kill -TERM pid`，或者不加`-TERM`暴力快速关闭。`-9`是强制杀掉，但不会杀掉子进程
...
- 通用配置
  ...
  - `worker_processes`：工作进程的数量，默认1，通常cpu数量或其2倍（新版本默认值是auto了），面试时回答中间值，（8核CPU的话就是6个进程）
...
- 配置
  ...
  upstream www.myweb.com {
  	...
  	#backup作用：更新服务器代码先在备份上更新，然后关闭一批服务器，更新它们，然后关闭另一批同时启动更新好的，用户连		接到backup和更新好的，待其他服务器也更新后，再启动其他服务器。即热部署
...
- 策略
  ...
  - 轮询
    ...
    > 安装上面的配置两个服务器轮流访问，要保证多个机器的性能、处理事务要一致
    >
